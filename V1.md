...
- [Concepts clés de la programmation fonctionnelle](#concepts-clés-de-la-programmation-fonctionnelle)
...
Le but du module 323 « Programmer de manière fonctionnelle » est de te rendre capable de **penser déclaratif** et de construire des solutions **claires, testables et réutilisables**.
...
**Comment lire ce RP (pense‑bête) :**
- chaque entrée = *déf courte* → *exemple minimal* → *variantes/astuce* ;
- blocs **🔗 Interconnexions** pour montrer comment combiner les notions (ex. `filter` → `map` → `reduce`, `Set` + `filter`, `JSON.parse` + `map`, etc.) ;
- le document est **imprimable** et utilisable à l’évaluation.
...
Veillez à ne pas utiliser `map()` pour **modifier** directement les éléments originaux. Les diapositives soulignent qu'une fonction passée à `map()` doit retourner un nouvel objet plutôt que muter l'existant. Si vous devez enrichir des objets, créez-en une copie :
...
const salairesElevés = salaires.filter(s => s > 75000); // [80000, 90000]
```
...
La **récursion** désigne une fonction qui s'appelle elle‑même jusqu'à atteindre un cas de base. Elle remplace souvent les boucles dans des langages fonctionnels et est utilisée pour parcourir des structures arborescentes ou implémenter des algorithmes classiques (factorielles, suites de Fibonacci, recherche dans un arbre). Par exemple, un compte à rebours récursif :
...
Les langages généralistes comme JavaScript ont des limites de taille de pile. Des appels récursifs trop profonds peuvent provoquer un **stack overflow**. Si le moteur de votre langage ne supporte pas l'optimisation de **récursion terminale**, privilégiez des approches itératives.
...
**Fonctions de première classe** : en PF, les fonctions sont des valeurs. On peut les stocker dans des variables, les passer en argument et les retourner comme résultats. Les fonctions d'ordre supérieur prennent une fonction en paramètre ou en retournent une.

**Immuabilité** : l'état est considéré comme immuable. Au lieu de modifier une variable, on crée de nouvelles valeurs. Cette approche évite les effets de bord et simplifie les tests, bien qu'elle puisse consommer plus de mémoire.

**Fonctions pures** : une fonction pure produit toujours la même sortie pour une même entrée et ne modifie pas d'état externe. Elles favorisent la composition et la réutilisation.

**Paradigmes impératif vs déclaratif** : l'approche impérative décrit comment faire en détaillant les étapes, tandis que l'approche déclarative décrit le résultat souhaité et laisse l'implémentation gérer le comment. La programmation fonctionnelle appartient à la famille déclarative.

Les sources de cette section proviennent des supports de cours.
# Conclusion
